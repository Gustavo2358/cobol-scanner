...
public final class Operands {
  ...
  public static Parsed read(List<Token> ts, int i) {
    ...
    Token t = ts.get(i++);
    if (t.kind() == Token.Kind.STRING || t.kind() == Token.Kind.NUMBER)
      return new Parsed(new Value.Literal(t.value()), i);
    if (t.is("SPACE") || t.is("SPACES")) return new Parsed(new Value.Literal(" "), i);
    if (t.is("ZERO") || t.is("ZEROS") || t.is("ZEROES"))
      return new Parsed(new Value.Literal("0"), i);
    if (t.is("FUNCTION")) {
      Parsed argument = read(ts, i);
      return new Parsed(new Value.Unknown("FUNCTION"), argument.next());
    }
    if (t.kind() != Token.Kind.WORD || FactScanner.BOUNDARIES.contains(t.upper()))
      return new Parsed(new Value.Unknown("Unsupported operand: " + t.text()), i);
    Value value = new Value.Ref(t.upper());
    while (i < ts.size()) {
      if ((ts.get(i).is("OF") || ts.get(i).is("IN"))
          && i + 1 < ts.size()
      ...
      if (colon >= 0) {
        ...
        Integer length =
            colon + 1 == i
                ? Integer.valueOf(-1)
                : colon + 2 == i ? integer(ts.get(colon + 1)) : null;
  ...
  private static Integer integer(Token t) {
    ...
    try {
      return Integer.valueOf(t.value());
