...
public final class Scanner {
    ...
    public Scanner(Options options) { this.options = options; preparation = new SourcePreparation(options); }
    public ScanResult scan(Path source, String label) {
        ScanResult r = new ScanResult(label); long start = System.nanoTime(), phase = start;
        try {
            r.bytes = Files.size(source);
            if(r.bytes > options.sourceBudget()) throw new java.io.IOException("Source byte budget exceeded: " + options.sourceBudget());
            try(var lines=Files.lines(source,options.charset)) { r.loc=lines.count(); }
            var tokens = preparation.prepare(source, r);
            r.preparationNanos=System.nanoTime()-phase; phase=System.nanoTime();
            ...
            if (r.programResolutionIncomplete) r.partial("Unresolved program target, source gap or value budget/cycle");
        } catch (Exception e) {
            r.scanStatus = "ERROR"; r.programResolutionIncomplete = true;
            r.diagnostics.add(e.getClass().getSimpleName()+": "+e.getMessage());
        } finally { r.totalNanos=System.nanoTime()-start; }
        return r;
    }
}
