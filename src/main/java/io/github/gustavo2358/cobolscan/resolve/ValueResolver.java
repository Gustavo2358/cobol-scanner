...
public final class ValueResolver {
  ...
  public ValueResolver(ValueFacts facts) {
    this(facts, 4096);
  }

  public ValueResolver(ValueFacts facts, int maxCandidates) {
    this(facts, maxCandidates, 65536, 8 * 1024 * 1024);
  }
  ...
  public ValueResolver(ValueFacts facts, int maxCandidates, int maxValueChars, long maxMemoBytes) {
    ...
    this.maxMemoBytes = maxMemoBytes;
  }
  ...
  private static final class Frame {
    ...
    Frame(Value value, List<Value> children) {
      this.value = value;
  ...
  public Resolution resolve(Value root) {
    ...
    Set<Value> active = new HashSet<>();
    stack.push(new Frame(root, children(root)));
    active.add(root);
    while (!stack.isEmpty()) {
      Frame f = stack.peek();
      if (f.next < f.children.size()) {
        ...
        if (memo.containsKey(child)) f.resolved.add(memo.get(child));
          ...
          f.cycle = true;
          f.resolved.add(empty(true));
        } else {
          stack.push(new Frame(child, children(child)));
          active.add(child);
        }
        continue;
      }
      Resolution result = evaluate(f);
      stack.pop();
      active.remove(f.value);
      // A context-truncated result must never poison another sink's memo.
      if (!f.cycle) cache(f.value, result);
      if (stack.isEmpty()) return result;
      Frame parent = stack.peek();
      parent.resolved.add(result);
      parent.cycle |= f.cycle;
    }
  ...
  private Resolution evaluate(Frame f) {
    ...
    boolean incomplete = f.cycle;
    for (Resolution r : f.resolved) incomplete |= r.incomplete;
    if (f.value instanceof Value.Literal l) {
      if (l.text().length() <= maxValueChars) values.add(l.text());
      ...
      for (Resolution part : f.resolved) {
        ...
        for (String prefix : values)
          for (String suffix : part.values) {
            if (next.size() >= maxCandidates) {
              incomplete = true;
              break outer;
            }
            if ((long) prefix.length() + suffix.length() > maxValueChars) {
              incomplete = true;
              continue;
            }
      ...
      if (s.start() == null
          ...
          || s.start() < 1
          || s.length() == 0
          || s.length() < -1) incomplete = true;
      else
        for (String v : f.resolved.get(0).values) {
      ...
      for (String v : f.resolved.get(0).values) {
        int pos = d.delimiter().isEmpty() ? -1 : v.indexOf(d.delimiter());
        values.add(pos < 0 ? v : v.substring(0, pos));
      }
    } else {
      for (Resolution r : f.resolved)
        for (String v : r.values) {
          if (values.size() < maxCandidates || values.contains(v)) values.add(v);
          else incomplete = true;
        }
    }
    return new Resolution(
        Collections.unmodifiableSortedSet(values), incomplete || values.isEmpty());
  }

  private long weight(Resolution r) {
    long n = 64;
    for (String v : r.values) n += 48L + 2L * v.length();
    return n;
  }

  private void cache(Value value, Resolution result) {
    long size = weight(result);
    if (size > maxMemoBytes) return;
    while (memoBytes + size > maxMemoBytes && !memo.isEmpty()) {
      var it = memo.entrySet().iterator();
      var first = it.next();
      memoBytes -= weight(first.getValue());
      it.remove();
    }
    Resolution previous = memo.put(value, result);
    if (previous != null) memoBytes -= weight(previous);
    memoBytes += size;
  }

  private static Resolution empty(boolean incomplete) {
    return new Resolution(Collections.emptySortedSet(), incomplete);
  }
}
