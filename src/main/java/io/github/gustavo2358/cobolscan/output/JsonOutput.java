...
public final class JsonOutput implements AutoCloseable {
  ...
  public JsonOutput(OutputStream out) throws IOException {
    ...
    json.writeArrayFieldStart("programs");
  }

  public void write(ScanResult r) throws IOException {
    json.writeStartObject();
    json.writeStringField("source", r.source);
    array("programs", r.programs);
    array("externalFileNames", r.externalFileNames);
    array("tables", r.tables);
    array("copybooks", r.copybooks);
    array("sqlIncludes", r.sqlIncludes);
    array("dclgens", r.dclgens);
    json.writeBooleanField("programResolutionIncomplete", r.programResolutionIncomplete);
    json.writeStringField("scanStatus", r.scanStatus);
    json.writeEndObject();
  }

  private void array(String name, Collection<String> values) throws IOException {
    json.writeArrayFieldStart(name);
    for (String v : values) json.writeString(v);
    json.writeEndArray();
  }
