...
public final class SqlScanner {
    private static final Set<String> STOP=Set.of("WHERE","GROUP","ORDER","HAVING","UNION","EXCEPT","INTERSECT","FETCH","FOR","SET","VALUES","WHEN","ON","RETURNING","INTO","SELECT","TABLE","FINAL","OLD","NEW","LATERAL","OF");
    ...
    private void region(List<Token> ts,ScanResult result) {
        ...
        boolean merge=ts.get(0).is("MERGE"), from=false, query=ts.get(0).is("DELETE"); Deque<Boolean> scopes=new ArrayDeque<>(), queries=new ArrayDeque<>();
        for(int i=0;i<ts.size();i++) {
            Token t=ts.get(i);
            if(t.is("(")) { scopes.push(from); queries.push(query); from=false; query=false; continue; }
            if(t.is(")")) { from=scopes.isEmpty()?false:scopes.pop(); query=queries.isEmpty()?false:queries.pop(); continue; }
            if(t.is("SELECT")) query=true;
            if(STOP.contains(t.upper()) && t.kind()==Token.Kind.WORD) from=false;
            int target=-1;
            if(t.is("FROM") && query) { from=true; target=i+1; }
            else if(t.is("JOIN")) target=i+1;
            else if(t.is(",") && from) target=i+1;
            else if(t.is("UPDATE") && (i==0 || !ts.get(i-1).is("FOR")) && i+1<ts.size() && !ts.get(i+1).is("SET") && !ts.get(i+1).is("OF")) target=i+1;
