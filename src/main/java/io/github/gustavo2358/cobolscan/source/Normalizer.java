...
public final class Normalizer {
    public static String normalize(String raw, String format, ScanResult result) {
        ...
        StringBuilder out = new StringBuilder(raw.length()); boolean fixed = format.equals("fixed"); char quote = 0;
        for (String line : raw.split("\n", -1)) {
            String body = line; boolean continuation = false;
            if (fixed) {
                if (line.length() < 7) { out.append('\n'); continue; }
                char indicator = line.charAt(6);
                if (indicator == '*' || indicator == '/') { out.append('\n'); continue; }
                if (indicator == 'D' || indicator == 'd') { /* Include debug lines conservatively. */ }
    ...
    private static char quoteAfter(String s, char quote) {
        for (int i = 0; i < s.length(); i++) {
            ...
            else if (quote == c) { if (i+1 < s.length() && s.charAt(i+1) == c) i++; else quote = 0; }
        }
        return quote;
    }
}
